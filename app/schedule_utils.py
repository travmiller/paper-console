...
def normalize_schedule_rules(
    ...
    for raw_rule in rule_payloads:
        ...
        if first.startswith("CRON_TZ=") or first.startswith("TZ="):
            expression_text = " ".join(parts[1:]).strip()

        cron_expression, _timezone_name = parse_cron_with_timezone(
            expression_text,
            resolved_fallback,
        )
        ...
        rules.append(
            {
                "expression": cron_expression,
                "timezone": resolved_fallback,
                "enabled": enabled,
                "description": humanize_cron(cron_expression, resolved_fallback),
            }
        )
