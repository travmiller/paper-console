...
def parse_events(ics_content: str, days_to_show: int, timezone_str: str) -> Dict[date, List[Dict]]:
    """
        ...
        if component.name == "VEVENT":
            ...
            if 'RRULE' in component:
                ...
                try:
                    ...
                    # Create rule
                    rule = rrulestr(component['RRULE'].to_ical().decode(), dtstart=rrule_start)
                    ...
                    print(f"[CALENDAR] RRULE Error for {summary}: {e}")
            ...
            for evt_dt, evt_is_all_day in event_instances:
                # Check bounds
                evt_date = evt_dt if isinstance(evt_dt, date) and not isinstance(evt_dt, datetime) else evt_dt.date()
...
def format_calendar_receipt(printer: PrinterDriver, config: CalendarConfig, module_name: str = None):
    ...
    for url in sources:
        ...
        if ics_data:
            ...
            print(f"[CALENDAR] Failed to fetch: {url}")
