"""
Integration tests validating scheduler_loop works correctly with new cron system.

These tests ensure the scheduler correctly evaluates both legacy HH:MM schedules
and new cron rule schedules with proper timezone handling.
"""
...
class TestSchedulerLoopIntegration:
    ...
    def test_timezone_override_per_rule(self):
        """Rule can override default timezone with CRON_TZ prefix."""
        ...
        assert extracted_tz == "Europe/London"
...
class TestSchedulePayloadHandling:
    ...
    def test_api_payload_with_new_cron_format(self):
        """API receiving new cron rule format should work."""
        payload = {
            'rules': [
                {
                    'expression': '0 9 * * MON-FRI',
        ...
        # First rule uses explicit per-rule timezone
        assert normalized[0]['timezone'] == 'America/Chicago'
        # Second rule overrides with CRON_TZ prefix
