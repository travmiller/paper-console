...
class TestCronParsing:
    """Test cron expression parsing with optional timezone prefix."""
    ...
    def test_parse_cron_with_cron_tz_prefix(self):
        """Cron with CRON_TZ= prefix should extract timezone."""
        cron_expr, tz_name = schedule_utils.parse_cron_with_timezone(
    ...
    def test_parse_cron_with_tz_prefix(self):
        """Cron with TZ= prefix should extract timezone."""
        cron_expr, tz_name = schedule_utils.parse_cron_with_timezone(
    ...
    def test_parse_cron_invalid_timezone_in_prefix_falls_back(self):
        """Invalid timezone in prefix should fall back to fallback."""
        cron_expr, tz_name = schedule_utils.parse_cron_with_timezone(
...
class TestNormalizeScheduleRules:
    ...
    def test_normalize_cron_tz_prefix(self):
        """Rule with CRON_TZ= prefix should extract timezone."""
        payloads = [
            {
    ...
    def test_normalize_missing_timezone_raises(self):
        """Rule without timezone (and without inline TZ prefix) should fail."""
        payloads = [
            {
                "expression": "0 9 * * *",
            }
        ...
        with pytest.raises(ValueError, match="requires a timezone"):
            schedule_utils.normalize_schedule_rules(payloads, "UTC")
    ...
    def test_normalize_preserves_description(self):
        ...
        rules = schedule_utils.normalize_schedule_rules(payloads, "UTC")
        assert rules[0]["description"] == "Custom description"
    ...
    def test_normalize_timezone_precedence(self):
        """Explicit timezone should override fallback."""
        payloads = [
            {
                "expression": "0 9 * * *",
        ...
        rules = schedule_utils.normalize_schedule_rules(payloads, "UTC")
        assert rules[0]["timezone"] == "America/Los_Angeles"
