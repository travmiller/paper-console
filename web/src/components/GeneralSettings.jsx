...
// Telegram Bot Settings Section Component
const TelegramBotSection = ({ settings, saveGlobalSettings, inputClass, labelClass, inkGradients }) => {
  const [botStatus, setBotStatus] = useState(null);
  const [isRestarting, setIsRestarting] = useState(false);
  const [statusMessage, setStatusMessage] = useState({ type: '', message: '' });

  // Get telegram_bot config with defaults
  ...
  const handleSave = (updates) => {
    saveGlobalSettings({
      telegram_bot: { ...telegramConfig, ...updates }
    });
  };
  ...
  const handleRestart = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.running) {
        setStatusMessage({ type: 'success', message: 'Bot restarted successfully!' });
      } else {
        setStatusMessage({ type: 'info', message: data.message });
      }
      // Refresh status
      ...
      setStatusMessage({ type: 'error', message: 'Failed to restart bot: ' + err.message });
    } finally {
  ...
  return (
    <div className='rounded-xl p-[4px] shadow-lg' style={{ background: inkGradients[6] || inkGradients[0] }}>
      <div className='bg-bg-card rounded-lg p-4 flex flex-col'>
        <h3 className='font-bold text-black text-lg tracking-tight mb-3'>Telegram Bot</h3>
        <p className='text-sm text-gray-600 mb-4'>
          Chat with AI through Telegram and print responses directly to your PC-1.
        </p>
        ...
        {botStatus && (
          <div className={`mb-4 p-3 rounded-lg text-sm border-2 ${
            ...
            <div className='flex items-center justify-between'>
              ...
              {botStatus.running && botStatus.users_connected > 0 && (
                <span className='text-xs text-gray-600'>
                  {botStatus.users_connected} active {botStatus.users_connected === 1 ? 'user' : 'users'}
                </span>
              )}
            </div>
            {!botStatus.available && (
        ...
        <div className='mb-4'>
          <label className='flex items-center gap-3 cursor-pointer'>
            ...
            />
            <span className='font-bold text-black'>Enable Telegram Bot</span>
          </label>
        </div>
        ...
        <div className='mb-4'>
          ...
          <input
            ...
            onChange={(e) => handleSave({ bot_token: e.target.value })}
            placeholder='123456:ABC-DEF1234ghI...'
        ...
        <div className='mb-4'>
          ...
          <input
            type='text'
            value={(telegramConfig.allowed_user_ids || []).join(', ')}
            onChange={(e) => handleSave({ allowed_user_ids: parseUserIds(e.target.value) })}
            placeholder='123456789, 987654321'
            className={inputClass}
          />
          <p className='text-xs text-gray-600 mt-1'>
            Comma-separated list. Leave empty to allow anyone. Send /id to the bot to get your ID.
          </p>
        </div>
        ...
        <div className='mb-4'>
          ...
          <select
            value={telegramConfig.ai_provider}
        ...
        {/* AI API Key */}
        <div className='mb-4'>
          <label className={labelClass}>AI API Key</label>
          <input
            type='password'
            value={telegramConfig.ai_api_key}
            onChange={(e) => handleSave({ ai_api_key: e.target.value })}
            placeholder={telegramConfig.ai_provider === 'anthropic' ? 'sk-ant-...' : 'sk-...'}
            className={inputClass}
          />
          <p className='text-xs text-gray-600 mt-1'>
            {telegramConfig.ai_provider === 'anthropic' 
        ...
        <div className='mb-4'>
          <label className={labelClass}>Custom Model (Optional)</label>
          <input
            type='text'
            value={telegramConfig.ai_model || ''}
            onChange={(e) => handleSave({ ai_model: e.target.value })}
            placeholder={telegramConfig.ai_provider === 'anthropic' ? 'claude-sonnet-4-20250514' : 'gpt-4o-mini'}
            className={inputClass}
          />
          <p className='text-xs text-gray-600 mt-1'>
        ...
        <PrimaryButton
          ...
          disabled={isRestarting || !telegramConfig.bot_token}
          loading={isRestarting}
          className='w-full'
        >
          {isRestarting ? 'Restarting...' : 'Apply & Restart Bot'}
        </PrimaryButton>
        ...
        {statusMessage.message && (
          <div className={`mt-4 p-3 rounded-lg text-sm border-2 ${
            ...
            {statusMessage.type === 'error' && <span className='font-bold mr-2'>ERROR:</span>}
            {statusMessage.message}
          </div>
        )}
      </div>
